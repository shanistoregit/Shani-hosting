...
def load_users():
    if not os.path.exists(USERS_FILE):
        default = {"shayan": {"password": "pak112233", "role": "admin"}}
        save_users(default)
        return default
    ...
    data = {}
    changed = False
    for raw_username, user_data in raw_data.items():
        username = str(raw_username).strip().lower()
        if username != raw_username:
            changed = True
        if not isinstance(user_data, dict):
            continue
        password = str(user_data.get('password', '')).strip().lower()
        if user_data.get('password') != password:
            changed = True
        user_data['password'] = password
        if username in data:
            # Merge server lists if legacy records differed only by letter case.
            existing_servers = data[username].get('servers', [])
            incoming_servers = user_data.get('servers', [])
            if isinstance(existing_servers, list) and isinstance(incoming_servers, list):
                data[username]['servers'] = existing_servers + incoming_servers
            changed = True
        else:
            data[username] = user_data

    if 'shayan' not in data:
        data['shayan'] = {"password": "pak112233", "role": "admin"}
        changed = True
    if changed:
        save_users(data)
    return data
...
def run_bot(server_id, main_file='main.py', requirements_file='requirements.txt'):
    ...
    try:
        ...
        ACTIVE_PROCESSES[server_id] = proc
        ...
        def stream_output():
            try:
                ...
                if ACTIVE_PROCESSES.get(server_id) is proc:
...
def api_create_server():
    ...
    password = request.args.get('password', '').strip().lower()
...
def login():
    if request.method == 'POST':
        ...
        users = load_users()
        if username == 'shayan' and password == str(users.get('shayan', {}).get('password', '')).lower():
            session['user'] = 'shayan'
            session['role'] = 'admin'
...
def server_login(server_id):
    ...
    if request.method == 'POST':
        ...
        for uname, data in users.items():
            ...
            for s in servers:
                if isinstance(s, dict) and s.get('server_id') == server_id:
                    if username == uname.lower() and password == str(data.get('password', '')).lower():
                        session['user'] = uname
                        session['role'] = 'user'
...
def api_input():
    ...
    if not server_id:
        return jsonify({'status': 'error', 'msg': 'Server ID is required'}), 400
    proc = ACTIVE_PROCESSES.get(server_id)
    if not proc or proc.poll() is not None or proc.stdin is None:
        ACTIVE_PROCESSES.pop(server_id, None)
        return jsonify({'status': 'error', 'msg': 'No running process is waiting for input'}), 409
    try:
        proc.stdin.write(value + '\n')
        proc.stdin.flush()
        log_file = os.path.join(get_server_dir(server_id), 'output.log')
        with open(log_file, 'a', encoding='utf-8') as f:
            f.write(f"[{datetime.now().strftime('%I:%M:%S %p')}] > {value}\n")
        return jsonify({'status': 'success', 'msg': 'Input sent'})
    except (BrokenPipeError, OSError, ValueError):
        ACTIVE_PROCESSES.pop(server_id, None)
        return jsonify({'status': 'error', 'msg': 'The process is no longer running'}), 409
...
def api_change_password(server_id):
    ...
    data = request.get_json()
    current_password = str(data.get('current_password', '')).strip().lower()
    new_password = str(data.get('new_password', '')).strip().lower()
    ...
    if username in users:
        if str(users[username].get('password', '')).lower() == current_password:
